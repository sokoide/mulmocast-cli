...
class MulmocastApp {
    ...
    if (uniqueUserNameInput) {
      uniqueUserNameInput.addEventListener("change", (e) => {
        ...
        if (userName) {
          ...
          // Reconnect SSE without username
  ...
  async checkHealth() {
    UIComponents.showLoading("Health check in progress...");
    try {
      const result = await this.apiClient.checkHealth();
      UIComponents.showResult(`✅ Health check succeeded!\n${JSON.stringify(result, null, 2)}`);
    } catch (error) {
      UIComponents.showResult(`❌ Health check failed: ${error.message}`, "error");
    }
  }
