...
class GenerationForm {
  ...
  async generateScript() {
    ...
    // Validate username: only alphabets, numbers, and underscores
    if (!/^[a-zA-Z0-9_]+$/.test(uniqueUserName)) {
      UIComponents.showResult("❌ Unique User Name can only contain letters, numbers, and underscores", "error");
      return;
    }

    // Validate filename/story: only alphabets, numbers, and underscores
    if (filename && !/^[a-zA-Z0-9_-]+$/.test(filename)) {
      UIComponents.showResult("❌ Story name can only contain letters, numbers, underscores, and hyphens", "error");
      return;
    }
    ...
    try {
      ...
      UIComponents.showResult(`✅ Script generation successful!\n${JSON.stringify(result, null, 2)}`);
  ...
  async generateVideoFromFile(fileId) {
    ...
    UIComponents.disableButton(button, "🔄 Generating video...");

    UIComponents.showLoading(`Generating video... (File: ${fileId})`);
    try {
  ...
  async generatePdfFromFile(fileId) {
    const button = event.target;
    UIComponents.disableButton(button, "🔄 Generating PDF...");

    UIComponents.showLoading(`Generating PDF... (File: ${fileId})`);
    try {
